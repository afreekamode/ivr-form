...
500)});a.isEmptyObject(c.captionHide)||a(b).mouseout(function(){a(b).find(".caption_background").stop().animate(c.captionHide,500)});var e=c.interval;"kenburns"==c.effect&&(e=1);0!=e?(e=setTimeout(function(){a.slideshow.imagetransition(b)},e),a(b).data("timerId",e)):"random"==c.type&&(a.slideshow.next(c),a.slideshow.gotoImage(b,c.current));0!=c.fullscreen&&(a(window).resize(function(){a.slideshow.resize(b)}),a(window).load(function(){a.slideshow.resize(b)}));setTimeout(function(){a(b).trigger("activate",
0)},10)};a.slideshow.killTimer=function(b){b=a(b).data("timerId");clearInterval(b)};a.slideshow.updateNavigation=function(b,c,d){var e=a(b).data("state");a(b).parent().find(".pagination img:eq("+c+")").attr("src",e.pagination_img_default);a(b).parent().find(".pagination img:eq("+d+")").attr("src",e.pagination_img_active)};a.slideshow.gotoImage=function(b,c){a.slideshow.killTimer(b);var d=a(b).data("state");0!=d.interval&&(d.current--,0>d.current&&(d.current=d.images.length-1));c-1!=d.current&&(0==
d.interval&&(d.last=d.current),d.current=Math.min(c-1,d.images.length-1),d.interval=0,d.pagination&&a.slideshow.updateNavigation(b,d.last,d.current),a.slideshow.imagetransition(b),d.last=d.current)};a.slideshow.nextimage=function(b){state=a(b).data("state");0!=state.interval&&(state.current=state.last);state.interval=0;a.slideshow.next(state);a.slideshow.imagetransition(b)};a.slideshow.previmage=function(b){state=a(b).data("state");0!=state.interval&&(state.current=state.last);state.interval=0;0<=
state.current-1?(--state.current,state.last=state.current+1):(state.current=state.images.length-1,state.last=0);a.slideshow.imagetransition(b)};a.slideshow.next=function(a){if("sequence"==a.type)a.current+1<a.images.length?(a.current+=1,a.last=a.current-1):(a.current=0,a.last=a.images.length-1);else if("random"==a.type)for(a.last=a.current;a.current==a.last;)a.current=Math.floor(Math.random()*a.images.length)};a.slideshow.imagetransition=function(b){state=a(b).data("state");if(state.random){var c=
"fade slide puff blind drop zoom clip shuffle blast blocks rain swirl squares bars".split(" "),d=["left","up","down","right"],e=["horizontal","vertical"];state.effect=c[Math.floor(Math.random()*c.length)];if("slide"==state.effect||"drop"==state.effect)state.direction=d[Math.floor(Math.random()*d.length)];if("clip"==state.effect||"blinds"==state.effect||"bars"==state.effect)state.direction=e[Math.floor(Math.random()*e.length)]}for(c=0;c<state.images.length;c++)a(state.images[c]).css("display","none");
...
complete:function(){a(b).trigger("activate",state.active)}})):"newspaper"==state.effect?(a(state.images[state.last]).animate({opacity:"hide"},{duration:state.effectlength,easing:state.easing}),a(state.images[state.current]).css("zIndex","1"),a(state.images[state.current]).css({scale:0,rotate:720}),a(state.images[state.current]).animate({opacity:"show",rotate:0,scale:1},{duration:state.effectlength,easing:state.easing,complete:function(){a(b).trigger("activate",state.active)}})):"blast"==state.effect?
(a(state.images[state.last]).css("zIndex","1"),a(state.images[state.last]).css("zIndex","0"),a(state.images[state.last]).hide(state.effect,{easing:state.easing,pieces:16},state.effectlength),a(state.images[state.last]).css("visibility","visible"),a(state.images[state.current]).show(state.effect,{easing:state.easing,pieces:16},state.effectlength,function(){a(b).trigger("activate",state.active)})):"explode"==state.effect?(a(state.images[state.last]).css("zIndex","1"),a(state.images[state.last]).css("zIndex",
...
{easing:state.easing,fx:"none",reverse:state.reverse},state.effectlength,function(){a(b).trigger("activate",state.active)}),state.reverse=!state.reverse):"bars"==state.effect?a(state.images[state.current]).css("zIndex","1").show("bars",{pieces:state.count,direction:state.direction,easing:state.easing},state.effectlength,function(){a(b).trigger("activate",state.active)}):"bounce"==state.effect?a(state.images[state.current]).css("zIndex","1").show("bounce",{direction:state.direction},state.effectlength,
function(){a(b).trigger("activate",state.active)}):a(state.images[state.current]).css("zIndex","1").show(state.effect,{},state.effectlength,function(){a(b).trigger("activate",state.active)})}c=a(state.images[state.current]).data("title");a(b).find(".caption_text").text(c);0!=state.interval&&(a.slideshow.next(state),c=setTimeout(function(){a.slideshow.imagetransition(b)},state.interval),a(b).data("timerId",c))};a.slideshow.resize=function(b){var c=a(b).data("state"),d=0,e=0;"undefined"!=typeof window.innerWidth?
(d=window.innerWidth,e=window.innerHeight):"undefined"!=typeof document.documentElement&&"undefined"!=typeof document.documentElement.clientWidth&&0!=document.documentElement.clientWidth?(d=document.documentElement.clientWidth,e=document.documentElement.clientHeight):(d=document.getElementsByTagName("body")[0].clientWidth,e=document.getElementsByTagName("body")[0].clientHeight);4==c.fullscreen?(d=a(b).parent().innerWidth(),e=a(b).parent().innerHeight()):5==c.fullscreen&&(d=a(b).parent().innerWidth(),
e=a(b).innerHeight(),a(b).css("overflow","hidden"));"kenburns"==c.effect&&(c.fullscreen=1);1==c.fullscreen&&a(b).css("height",e);a(b).css("width","100%");a(b).css("left","0");5!=c.fullscreen&&a(b).css("top","0");a(b).find(".image").each(function(){var g=e,f=d,h=0,l=0;if(4==c.fullscreen)f>c.maxWidth&&(f=c.maxWidth),a(this).css({height:"auto",width:f+"px"}),a(this).css({position:"absolute",left:h+"px",top:l+"px"}),a(b).css("height",a(this).innerHeight()),d>f&&(h=(d-a(this).width())/2,a(this).css({left:h+
"px"}));else{var k=!1;(k=5==c.fullscreen?!(f/g<a(this).width()/a(this).height()):f/g>a(this).width()/a(this).height())?1==c.fullscreen?a(this).css({height:g+"px",width:"auto"}):2!=c.fullscreen&&5!=c.fullscreen||a(this).css({height:"auto",width:f+"px"}):1==c.fullscreen?a(this).css({height:"auto",width:f+"px"}):2!=c.fullscreen&&5!=c.fullscreen||a(this).css({height:g+"px",width:"auto"});1==c.fullscreen?(h=(d-a(this).width())/2,l=(e-a(this).height())/2):3==c.fullscreen&&a(this).css({height:g+"px",width:f+
"px"});5!=c.fullscreen&&a(this).css({position:"fixed",left:h+"px",top:l+"px"})}})}})(jQuery);